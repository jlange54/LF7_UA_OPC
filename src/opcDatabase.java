...
public class opcDatabase {
   ...
   public static void fillStationsSensors() throws Exception {
        Connection opcDatabase = DriverManager.getConnection("jdbc:mysql://localhost:3306/lf7_ua_opc", "root", "");
        fillSensors(opcDatabase);
   }

   private static boolean isTableFilled(Connection connection, String table) throws SQLException {
     ...
     * This method fills the opcDatabase table station with all stations except "Controller"
     * @param connection
     * @throws SQLException
     */
   private static void fillStations(Connection connection) throws Exception {
       if (isTableFilled(connection, "lf7_ua_opc.station")) {
     ...
     * This method fills the opcDatabase table sensor with all sensors according to their station
     * @param connection
     * @throws Exception
     */
   private static void fillSensors(Connection connection) throws Exception {
        if (isTableFilled(connection, "lf7_ua_opc.sensor")) {
            throw new Exception("Table is already filled");
        }

        List<Station> stationList = getStationsFromDB(connection);
   ...
   private static List<Station> getStationsFromDB(Connection connection) throws SQLException {
